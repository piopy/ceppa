...
for _var in _CRED_VARS + ("NO_GCE_CHECK", "GOOGLE_GENAI_USE_VERTEXAI",
                           "DATABASE_URL", "LLM_MODEL", "DEFAULT_LANGUAGE"):
    val = os.environ.get(_var, "<NOT SET>")
    if val not in ("<NOT SET>", "true", "false") and len(val) > 8:
        val = val[:4] + "***" + val[-4:]
    logger.warning(f"  {_var}={val}")

logger.warning("GOOGLE-AUTH MODULES IN sys.modules: %s",
...
def _get_client(user=None) -> AsyncOpenAI:
    ...
    if user and getattr(user, "custom_openai_api_key", None):
        ...
        if user:
            logger.warning("  user.custom_openai_api_key=%s",
